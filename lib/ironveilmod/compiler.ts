...
class IronVeilCompiler {
  ...
  private selectVmMode(): VmMode {
    const depth = this.currentDepth;
    if (depth <= 1) {
      return 0;
    }
    const roll = this.vmRng.nextInt(4);
    if (roll === 0) return 2;
    if (roll === 1) return 1;
    return 0;
  }
  ...
  private pushFunction(parameters: Identifier[], body: Statement[], fallbackVararg: boolean): number {
    ...
    const vmMode = this.selectVmMode();
    this.currentDepth += 1;
  ...
  private internString(value: string): number {
    ...
    const existing = this.stringPool.get(safe);
    if (existing) {
      return existing;
    }
    const index = this.strings.length + 1;
    this.stringPool.set(safe, index);
    this.strings.push(safe);
    return index;
  }
